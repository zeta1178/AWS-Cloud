...
1. In the Find Services search box, type S3, and then in the Services list that displays, click S3, as the following image shows:
...
3. On the “Create bucket” page, in the “General configuration” area, set the details as follows:
    ...
    * In the “Bucket name” box, type “fintech-bootcamp-activities-,” followed by the initials of your name, then another hyphen, and then the current date using hyphens to separate digits. An example of what type in the box is the following:
...
4. Leave the rest of the options with their default values, scroll down to the bottom of the page, and then click the “Create bucket” button, as the following image shows:

    ![A screenshot points out the “Create bucket” button](Images/15-0-creating-S3-bucket-step-4.png)

5. Note that a “Successfully created bucket” confirmation message displays, as the following image shows:
...
* Walk-through the solved notebook, cell by cell, highlighting the following points:
  ...
  * Despite using a "curated" dataset, we still need to perform some data preparation tasks: _split_, _hot-encode_ and _scale_ the input features.
